...
Twelve Data's free tier is **8 API credits/min, 800/day** (1 credit per symbol). The proxy tracks **both** rolling windows (`TWELVEDATA_CREDITS_PER_MIN` default 8, `TWELVEDATA_DAILY_LIMIT` default 800) and spends up to whichever has less headroom; when neither has room it serves cache/seed. It also caps quotes refreshed **per poll** (`TWELVEDATA_MAX_REFRESH_PER_REQUEST`, default 6) so a single request never claims the whole minute (avoids the 8/8 burst, leaves room for indices), and keeps an in-memory quote cache (`QUOTE_TTL_MS`, default 120s). Each request serves cached values immediately and refreshes only the stalest symbols within budget.
